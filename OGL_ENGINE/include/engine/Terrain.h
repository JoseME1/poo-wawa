...
public:
	...
	float Superficie(float x, float z)
	{
		...
		if (difx < difz)
		{
			...
			glm::vec3 v1(vertices[indiceix + 1 + (indiceiz + 1) * width].Position.x - vertices[indiceix + indiceiz * width].Position.x,
				vertices[indiceix + 1 + (indiceiz + 1) * width].Position.y - vertices[indiceix + indiceiz * width].Position.y,
				vertices[indiceix + 1 + (indiceiz + 1) * width].Position.z - vertices[indiceix + indiceiz * width].Position.z);

			glm::vec3 v2(vertices[indiceix + 1 + indiceiz * width].Position.x - vertices[indiceix + indiceiz * width].Position.x,
				vertices[indiceix + 1 + indiceiz * width].Position.y - vertices[indiceix + indiceiz * width].Position.y,
				vertices[indiceix + 1 + indiceiz * width].Position.z - vertices[indiceix + indiceiz * width].Position.z);


			glm::vec3  normalPlano = glm::cross(v1, v2);
			//entonces solo falta calcular D
			D = -1 * (normalPlano.x * vertices[indiceix + indiceiz * width].Position.x +
				normalPlano.y * vertices[indiceix + indiceiz * width].Position.y +
				normalPlano.y * vertices[indiceix + indiceiz * width].Position.z);
			//sustituyendo obtenemos la altura de contacto en el terreno
			altura = ((-normalPlano.x * x - normalPlano.z * z - D) / normalPlano.y);
		}
		else
		{
			glm::vec3 v1(vertices[indiceix + (indiceiz + 1) * width].Position.x - vertices[indiceix + indiceiz * width].Position.x,
				vertices[indiceix + (indiceiz + 1) * width].Position.y - vertices[indiceix + indiceiz * width].Position.y,
				vertices[indiceix + (indiceiz + 1) * width].Position.z - vertices[indiceix + indiceiz * width].Position.z);

			glm::vec3 v2(vertices[indiceix + 1 + (indiceiz + 1) * width].Position.x - vertices[indiceix + indiceiz * width].Position.x,
				vertices[indiceix + 1 + (indiceiz + 1) * width].Position.y - vertices[indiceix + indiceiz * width].Position.y,
				vertices[indiceix + 1 + (indiceiz + 1) * width].Position.z - vertices[indiceix + indiceiz * width].Position.z);
			...
			D = -1 * (normalPlano.x * vertices[indiceix + indiceiz * width].Position.x +
				normalPlano.y * vertices[indiceix + indiceiz * width].Position.y +
				normalPlano.z * vertices[indiceix + indiceiz * width].Position.z);
